...
class Wolf_Popup_Options {
	...
	public function __construct( $settings = array() ) {
		...
		if ( class_exists( 'Wolf_Core' ) ) {
			...
			$this->extension_prefix = 'wolf-core';

		} elseif ( class_exists( 'Wolf_Visual_Composer' ) ) {

			$this->extension_prefix = 'wvc';
		}
	}
	...
	public function setting_field( $args ) {
		...
		if ( 'text' == $type || 'url' == $type ) {
			...
			?>
			<textarea class="large-text" name="<?php echo esc_attr( $settings_slug . '[' . $field_id . ']' ); ?>" rows="5"><?php echo esc_attr( wolf_popup_get_option( $settings_slug, $field_id ) ); ?></textarea>
			<?php
		} elseif ( 'editor' === $type ) {
			...
			foreach ( $pages as $page ) {
				...
				if ( get_post_field( 'post_parent', $page->ID ) ) {
					$page_options[ absint( $page->ID ) ] = '&nbsp;&nbsp;&nbsp; ' . esc_attr( $page->post_title );
				} else {
					$page_options[ absint( $page->ID ) ] = esc_attr( $page->post_title );
				}
			}
			...
			<select name="<?php echo esc_attr( $settings_slug . '[' . $field_id . ']' ); ?>">
				<?php foreach ( $page_options as $id => $title ) : ?>
					<option value="<?php echo absint( $id ); ?>" <?php selected( $value, $id ); ?>><?php echo esc_attr( $title ); ?></option>
				<?php endforeach; ?>
			</select>
			<?php
			...
			<select name="<?php echo esc_attr( $settings_slug . '[' . $field_id . ']' ); ?>">
				<?php if ( array_keys( $choices ) != array_keys( array_keys( $choices ) ) ) : ?>
					<?php foreach ( $choices as $key => $name ) : ?>
						<option value="<?php echo esc_attr( $key ); ?>" <?php selected( $value, $key ); ?>><?php echo esc_attr( $name ); ?></option>
					<?php endforeach; ?>
				<?php else : ?>
					<?php foreach ( $choices as $choice ) : ?>
						<option value="<?php echo esc_attr( $choice ); ?>" <?php selected( $value, $choice ); ?>><?php echo esc_attr( $choice ); ?></option>
					<?php endforeach; ?>
				<?php endif; ?>
			</select>
			<?php
			...
			$colorpicker_id = uniqid( $this->extension_prefix . '-settings-colorpicker-' );
			?>
			<script>
			...
			<input id="<?php echo esc_attr( $colorpicker_id ); ?>" value="<?php echo wolf_popup_sanitize_color( wolf_popup_get_option( $settings_slug, $field_id ) ); ?>" name="<?php echo esc_attr( $settings_slug . '[' . $field_id . ']' ); ?>" class="<?php echo esc_attr( $this->extension_prefix ); ?>-settings-colorpicker">
			<?php
		} elseif ( 'image' === $type ) {
			/**
			 * Bg image
			...
			if ( ! $image_id ) {
				echo 'display:none;';}
			?>
			" class="<?php echo esc_attr( $this->extension_prefix ); ?>-img-preview" src="<?php echo esc_url( $image_url ); ?>" alt="<?php echo esc_attr( $field_id ); ?>">
			<br>
			<a href="#" class="button <?php echo esc_attr( $this->extension_prefix ); ?>-reset-img"><?php esc_html_e( 'Clear', 'wolf-popup' ); ?></a>
			<a href="#" class="button <?php echo esc_attr( $this->extension_prefix ); ?>-set-img"><?php esc_html_e( 'Choose an image', 'wolf-popup' ); ?></a>
			<?php
			...
			<p>
				...
				<input value="<?php echo wolf_popup_sanitize_color( $color ); ?>" name="<?php echo esc_attr( $settings_slug . '[' . $field_id . '][color]' ); ?>" class="<?php echo esc_attr( $this->extension_prefix ); ?>-settings-colorpicker">
			</p>
			<?php
			/**
			 * Bg image
			...
			<p>
				...
				<select name="<?php echo esc_attr( $settings_slug . '[' . $field_id . '][repeat]' ); ?>">
					<?php foreach ( $options as $option ) : ?>
						<option <?php selected( $repeat, $option ); ?>><?php echo esc_attr( $option ); ?></option>
					<?php endforeach; ?>
				</select>
			</p>
			<?php
			...
			?>
			 <p>
				 ...
				 <select name="<?php echo esc_attr( $settings_slug . '[' . $field_id . '][position]' ); ?>">
					<?php foreach ( $options as $option ) : ?>
						<option <?php selected( $position, $option ); ?>><?php echo esc_attr( $option ); ?></option>
					<?php endforeach; ?>
				 </select>
			</p>
			 <?php
			...
			<p>
				...
				<select name="<?php echo esc_attr( $settings_slug . '[' . $field_id . '][size]' ); ?>">
					<?php foreach ( $options as $option => $display ) : ?>
						<option value="<?php echo esc_attr( $option ); ?>" <?php selected( $size, $option ); ?>><?php echo esc_attr( $display ); ?></option>
					<?php endforeach; ?>
				</select>
			</p>
			<?php
			...
			<p>
				...
				<select name="<?php echo esc_attr( $settings_slug . '[' . $field_id . '][attachment]' ); ?>">
					<?php foreach ( $options as $option ) : ?>
						<option <?php selected( $attachment, $option ); ?>><?php echo esc_attr( $option ); ?></option>
					<?php endforeach; ?>
				</select>
			</p>
			<?php
